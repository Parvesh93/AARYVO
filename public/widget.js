(() => {
  ...
  prechat.onsubmit=async(event)=>{
    ...
    const data=new FormData(prechat),name=String(data.get("name")||"").trim(),phone=String(data.get("phone")||"").trim(),email=String(data.get("email")||"").trim();
    preerror.textContent="";
    if(!name||(!phone&&!email)){preerror.textContent="Please enter your name and either a phone number or email.";return;}
    start.disabled=true;start.textContent="Saving…";
    try{
      const res=await fetch(`${base}/api/widget/contact`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({agentId,visitorId,name,phone,email})});
      const result=await res.json();if(!res.ok)throw new Error(result.error||"Unable to save your details.");
      conversationId=result.conversationId;lead=result.lead;contactCaptured=true;prechat.style.display="none";chat.classList.add("show");input.focus();
    }catch(err){preerror.textContent=err.message||"Unable to save your details. Please try again.";}finally{start.disabled=false;start.textContent="Start conversation →";}
  };

  form.onsubmit=async(event)=>{
    event.preventDefault();const text=input.value.trim();if(!text||!contactCaptured||!conversationId)return;add(text,"user");input.value="";input.disabled=true;const thinking=add("AARYVO is thinking…","ai");
    try{const res=await fetch(`${base}/api/widget/chat`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({agentId,visitorId,conversationId,message:text})});const data=await res.json();thinking.remove();if(!res.ok)throw new Error(data.error||"Unable to respond.");lead=data.lead||lead;add(data.reply,"ai");if(lead&&(lead.status==="HOT"||lead.status==="QUALIFIED"||lead.score>=65))book.style.display="block";}
  ...
  bookingButton.onclick=async()=>{
    if(!contactCaptured||!conversationId||!dateInput.value)return;bookingButton.disabled=true;
