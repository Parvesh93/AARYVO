...
export default async function EmailAnalyticsPage({searchParams}:{searchParams:SearchParams}){
  ...
  return <div>
    ...
    <div className="mt-4 overflow-hidden rounded-3xl border border-black/[.055] bg-white"><div className="border-b border-black/[.055] px-6 py-5"><h2 className="font-semibold">Top clicked links</h2><p className="mt-1 text-xs text-black/35">Which CTAs and destinations are generating engagement.</p></div><div className="overflow-x-auto"><table className="w-full min-w-[720px] text-left text-xs"><thead className="bg-black/[.018] text-[10px] uppercase tracking-[.12em] text-black/35"><tr><th className="px-6 py-3">Destination</th><th>Total clicks</th><th>Emails clicked</th><th>Last click</th></tr></thead><tbody>{d.topLinks.map(r=><tr key={r.url} className="border-t border-black/[.05]"><td className="max-w-[560px] px-6 py-4"><a href={r.url} target="_blank" rel="noreferrer" className="block truncate font-medium hover:underline">{r.url}</a></td><td>{r.clicks}</td><td>{r.uniqueEmails}</td><td>{dt(r.lastClickedAt)}</td></tr>)}{!d.topLinks.length&&<tr><td colSpan={4} className="px-6 py-8 text-center text-black/30">Link-level clicks from newly sent tracked emails will appear here.</td></tr>}</tbody></table></div></div>
