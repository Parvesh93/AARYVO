...
export async function PUT(request: Request) {
  ...
  try {
    ...
    const fromName = String(body.smtpFromName || "").trim();
    const fromEmail = String(body.smtpFromEmail || "").trim().toLowerCase();
    ...
    await prisma.business.update({
      ...
      data: {
        customSmtpEnabled: enabled,
        smtpHost: host || null,
        smtpPort: Number.isInteger(port) ? port : null,
        smtpSecure: body.smtpSecure === true,
        smtpUser: user || null,
        smtpPasswordEncrypted: password ? encryptToken(password) : member.business.smtpPasswordEncrypted,
    ...
    console.error("AARYVO email settings update error", error);
    return NextResponse.json({ error: "Email settings could not be saved." }, { status: 500 });
  }
}
...
export async function POST(request: Request) {
  ...
  try {
    ...
    await mail.transporter.sendMail({
      from: mail.from,
      to: recipient,
      subject: "AARYVO email delivery test",
      text: `Email delivery is working for ${member.business.name}. This test was sent using ${mail.source === "workspace" ? "your workspace SMTP settings" : "AARYVO email delivery"}.`,
      html: `<div style="font-family:Arial,sans-serif;max-width:560px;margin:auto;padding:32px"><div style="font-size:12px;font-weight:700;letter-spacing:.14em">AARYVO</div><h2 style="margin:18px 0 8px">Email delivery is working.</h2><p style="color:#667085;line-height:1.6">This test confirms email delivery for <strong>${member.business.name}</strong>.</p><p style="color:#667085;line-height:1.6">Delivery method: ${mail.source === "workspace" ? "Your workspace SMTP" : "AARYVO email delivery"}.</p></div>`,
    });

    return NextResponse.json({ ok: true, source: mail.source });
  } catch (error) {
    console.error("AARYVO test email error", error);
    const message = error instanceof Error ? error.message : "Unable to send test email.";
    return NextResponse.json({ error: `Test email failed: ${message}` }, { status: 400 });
  }
}
