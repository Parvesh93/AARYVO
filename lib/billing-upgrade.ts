...
// Only an unfinished checkout should block a new plan-change attempt.
// AUTHENTICATED/ACTIVE transitions represent successfully applied entitlements
// and must not make every later upgrade look like a duplicate checkout.
export async function activeTransitionForBusiness(businessId:string){await ensureBillingTransitionTable();const rows=await prisma.$queryRawUnsafe<Array<Record<string,unknown>>>(`SELECT * FROM billing_subscription_transition WHERE businessId = ? AND status = 'CREATED' ORDER BY createdAt DESC LIMIT 1`,businessId);return rowToTransition(rows[0])}

export async function latestAppliedTransitionForBusiness(businessId:string){await ensureBillingTransitionTable();const rows=await prisma.$queryRawUnsafe<Array<Record<string,unknown>>>(`SELECT * FROM billing_subscription_transition WHERE businessId = ? AND status IN ('AUTHENTICATED','ACTIVE') ORDER BY createdAt DESC LIMIT 1`,businessId);return rowToTransition(rows[0])}

export async function transitionByOldSubscription(oldSubscriptionId:string){await ensureBillingTransitionTable();const rows=await prisma.$queryRawUnsafe<Array<Record<string,unknown>>>(`SELECT * FROM billing_subscription_transition WHERE oldSubscriptionId = ? AND status IN ('AUTHENTICATED','ACTIVE','SUPERSEDED') ORDER BY createdAt DESC LIMIT 1`,oldSubscriptionId);return rowToTransition(rows[0])}
