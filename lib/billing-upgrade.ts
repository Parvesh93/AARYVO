...
function rowToTransition(row: Record<string, unknown> | undefined): BillingTransition | null {
  ...
  return {
    ...
    oldSubscriptionId: String(row.oldSubscriptionId || ""),
    newSubscriptionId: String(row.newSubscriptionId || ""),
    fromPlan: String(row.fromPlan || ""),
    toPlan: String(row.toPlan || ""),
    direction: String(row.direction || ""),
    upfrontPaise: Number(row.upfrontPaise || 0),
...
export async function transitionByNewSubscription(newSubscriptionId: string) {
  await ensureBillingTransitionTable();
  const rows = await prisma.$queryRawUnsafe<Array<Record<string, unknown>>>(
    "SELECT * FROM billing_subscription_transition WHERE newSubscriptionId = ? LIMIT 1",
    newSubscriptionId,
  );
  return rowToTransition(rows[0]);
}

export async function activeTransitionForBusiness(businessId: string) {
  await ensureBillingTransitionTable();
  const rows = await prisma.$queryRawUnsafe<Array<Record<string, unknown>>>(
    `SELECT * FROM billing_subscription_transition
     WHERE businessId = ? AND status IN ('CREATED','AUTHENTICATED','ACTIVE')
...
export async function transitionByOldSubscription(oldSubscriptionId: string) {
  await ensureBillingTransitionTable();
  const rows = await prisma.$queryRawUnsafe<Array<Record<string, unknown>>>(
    `SELECT * FROM billing_subscription_transition
     WHERE oldSubscriptionId = ? AND status IN ('AUTHENTICATED','ACTIVE')
     ORDER BY createdAt DESC LIMIT 1`,
    oldSubscriptionId,
  );
  return rowToTransition(rows[0]);
...
export async function updateBillingTransition(newSubscriptionId: string, status: string, paymentId?: string | null) {
  await ensureBillingTransitionTable();
  await prisma.$executeRawUnsafe(
    `UPDATE billing_subscription_transition
     SET status = ?, paymentId = COALESCE(?, paymentId), updatedAt = CURRENT_TIMESTAMP(3)
     WHERE newSubscriptionId = ?`,
    status,
    paymentId || null,
    newSubscriptionId,
  );
